    ...
    fs.readFile(filePath, "utf-8", (err, data) => {
      ...
      for (username in users) {
        const userPointRx = new Point("openvpn_status")
          ...
          .floatField("bytes_rx", users[username].rxBytes);
        userPoints.push(userPointRx);
        const userPointTx = new Point("openvpn_status")
          .tag("username", username)
          .tag("realAddress", users[username].realAddress)
          .tag("virtualAddress", users[username].virtualAddress)
          .floatField("bytes_tx", users[username].txBytes);
        userPoints.push(userPointTx);
        totalRxBytes += users[username].rxBytes;
        totalTxBytes += users[username].txBytes;
      }
      userPoints.push(
        new Point("openvpn_status").floatField("total_bytes_rx", totalRxBytes),
      );
      userPoints.push(
        new Point("openvpn_status").floatField("total_bytes_tx", totalTxBytes),
      );
